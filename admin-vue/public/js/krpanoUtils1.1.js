...
class KrpanoUtilsClass {
    ...
    addHotspots(spotname, style = "skin_prod_list", tiptext = '', hlookat = '', vlookat = '', dragable = true) {
        ...
        this.krpano.call("set(hotspot[" + spotname + "].onup,js(listen_hotspot(" + spotname + ")););");
        if (dragable) {
    ...
    registercontentsize(spotname, imagewidth, imageheight) {
        ...
        this.krpano.call("hotspot[" + spotname + "].updatepos()");
    }
    ...
    getViewFov() {
        return {
            "fov": this.krpano.get("view.fov"),
            "fovmin": this.krpano.get("view.fovmin"),
            "fovmax": this.krpano.get("view.fovmax"),
        }
    }
     ...
     * @param {*} fovmax 
     */
    setViewFov(fov, fovmin = null, fovmax = null) {
        this.krpano.set("view.fov", fov);
        // 设置最小缩放
        if (fovmin) {
            this.krpano.set("view.fovmin", fovmin);
        }
        // 设置最大缩放
        if (fovmax) {
